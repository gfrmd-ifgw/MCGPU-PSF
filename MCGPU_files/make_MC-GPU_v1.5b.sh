#!/bin/bash
# 
#   ** Simple script to compile the code MC-GPU_PSF v1.5 tested with CUDA 10.2 **
#
...
#                      @date    2017/06/28 
#                               2012/12/12
#                      @Modified by Rodrigo Massera
#                      @date    2020/06/16
#                      added the "-gencode=arch=compute_61,code=sm_61" flag as default
...
nvcc MC-GPU_v1.5b.cu -o MC-GPU_v1.5b.x -m64 -O3 -use_fast_math -DUSING_CUDA -DUSING_MPI -I. -I/usr/local/cuda/include -I/usr/local/cuda/samples/common/inc -I/usr/local/cuda/samples/shared/inc/ -I/usr/include/openmpi -L/usr/lib/ -lmpi -lz --ptxas-options=-v -gencode=arch=compute_61,code=sm_61 -gencode=arch=compute_50,code=sm_50 -gencode=arch=compute_30,code=sm_30 
